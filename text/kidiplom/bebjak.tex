...
na zdiaľanej pamäti, CSP je založené na komunikácií medzi procesmy ako hlavným mechanizmom koordinácie. Procesy si neposkytujú priamy
prístup k pamäti, čím sa znižuje počet chýb súbehu. Základnou myšlienkou CSP je, že program sa skladá z viacerých sekvečých procesov
vykonávaných súbežne, ktoré medzi sebou komunikú prostredníctvom vstupno-výstupných operácií.
...
\end{}
Koncept strážených príkazov zavedení Djikstrom umožňuje riadiť vetvenie a nedeterministické správanie. Paralelná kompozícia opisuje
...
pomocou synchronnej komunikácie, pri ktorej musí byť odosieľateľ aj príjimateľ pripravený na výmenu údajov v rovnakom okamihu.
Tento prístup eliminuje problémy spojené s chybamy súbehu a nekonzistentným stavom pamäte.

\subsection{Komunikácia v CSP}
Hlavnou vlastnosťou CS je prenos dát medzi procesmy. Hodnota odoslaná jedným procesom je priamo priradená premennej v druhom
procese. Prenos hodnoty je atomický

V CSP sú vstup a výstup základné operácie,
na rozdiel od tradičného prístupu, kde je základnou operáciou priradenie. Tietp operácie umožňujú procesom medzi sebou komunikovať a koordinovať
svoju činnosť. CSP umožňuje definovať paralelné vykonávanie viacerých procesov pomocou paralelného príkazu.
Všetky procesy sa vykonávajú súčasne a program pokračuje až po ich dokončení.
Komunikácia medzi procesmi v CSP je synchronná, čo znamená, že odosielateľ aj príjemca musia byť pripravení na komunikáciu v rovnakom čase.
Neexistuje žiadne implicitné bufferovanie správ, čím sa zabezpečuje deterministické správanie komunikácie. Model CSP predpokladá daný počet
procesov. Nepodporuje dynamické vytváranie procesov ani rekurziu, čo zjednodušuje model.
...
alebo odlišný index napríklad \uv{X(0)}. Každá komunikácia musí mať jednoznačne určný svoj zdroj alebo cieľ. Keď je monitor pripravený komunikovať
...
tak zvanú stráž. Napríklad dve po sebe idúce požiadavky od rovnakého procesu, môžu byť zablokované pomocou podmienky \uv{j=0}.
\[
...
Nie sú to vláklna operaščného systému ani vlákna riadene runtime prostredím. Ide o
vyššiu abstrakciu nazývanú korutiny. Korutiny sú súbežné podprogramy, ktoré nemôžu byť
prerušené zvonka. Obsahujú však body, v ktorých môžu byť pozastavené a znovu spustené.
Na rozdiel od vláken je prepínanie medzi gorutinami rýchlejšie približne o 1,242 $\mu\text{s}$.\cite{cox2017}
...
Goroutine si sami neurčujú bady pozastavenia. Runtime Go sleduje ich správanie a
...
\section{Select}
\section{Chat server}
Navrhnutý prototyp servru je založený na architektúre klient-server, kde server zabezpečuje výmenu správ medzi pripojenými
...
využívajúc jeho vstavanú podporu pre súbežné programovanie prostredníctvom goroutine. Hlavná úloha servera spočíva v príjmaní
nových klientských spojení, spracovaní ich požiadavkov a správe komunikačných miestností.
...
čaká na nové pripojenie, ak je pripojenie úspešné tak vytvorý pre to spojenie novú goroutine. Týmto je zabezpečené, že každý
klient je obslúžený súbežne a bez blokovania príjmania ďalších klientov.

Každý klient je obsúžený na samostatnej goroutine, ktorá vykonáva funkciu \texttt{HandleConnection}. Toto riešenie predstavuje
...
Po pripojení je od klienta vyžiadané meno. Následne v nekonečnom cykle prvá podmienka pripojí klienta do komukačnej
...
je umožnené ďalšiemu klientovy poslanie správy cez komunikačný kanál. Je preto klient
pozastavený a čaká kým je opäť program pripravený spracovať ďalšiu správu.
